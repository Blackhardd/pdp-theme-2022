                        ...
                        <select name="salon" data-icon="location" data-confirm="<?=__( 'Смена салона удалит все услуги из корзины. Вы уверенны?', 'pdp' ); ?>">
                            <?php foreach( $salons as $key => $salon ) : ?>
                                <?php if( carbon_get_post_meta( $salon['id'], 'display_in_booking' ) === 'yes' ) : ?>
                                    <option value="<?=$salon['id']; ?>" <?=( isset( $_GET['salonId'] ) && $_GET['salonId'] == $salon['id'] ) || ( !isset( $_GET['salonId'] ) && $salon['id'] == $main_salon ) || ( !isset( $_GET['salonId'] ) && !$main_salon && $key === 0 ) ? 'selected' : ''; ?>><?="{$salon['city']}, {$salon['name']}"; ?></option>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </select>
                    </div>
